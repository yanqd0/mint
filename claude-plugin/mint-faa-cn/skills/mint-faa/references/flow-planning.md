...
触发：版本 / 计划 / 里程碑 / roadmap / milestone / plan / sprint / 拆解执行计划 / 方案执行。

## 步骤

1. **版本规划**（roadmap / milestone）：`roadmap create "<版本标题>" --version <V> --body "<目标+范围+验收>"`（version 必填、语义化；
   登记前 `roadmap list --all` 按 version 查重，**重复则不加、不问**）。
2. **执行计划**（plan / sprint）：`plan create "<计划标题>" --body "<body>" --roadmap <RM>`。
