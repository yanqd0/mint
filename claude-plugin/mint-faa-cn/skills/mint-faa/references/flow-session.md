...
1. **概览**：`list --json` 拉当前 open/planned 概览，`roadmap list --all` / `plan list --all` 看规划现状。
...
3. **roadmap/milestone 检查与建议**：对比现有 roadmap 与项目当前状态，若发现新的版本规划迹象
   （如代码里出现下一版本需求/方向）→ **向用户确认后** `roadmap create`（重复则不问、不加）。
4. **下一步计划建议**：基于 roadmap 规划 + open issues，推荐下一个应开发项，附理由：
