...
1. **Version planning** (milestone / milestone): `milestone create "<title>" --version <V> --body "<goal+scope+acceptance>"` (version required, semver;
   search `milestone list --all-states` by version before creating; skip if duplicate, don't ask).
2. **Execution plan** (plan / sprint): `plan create "<title>" --body "<body>" --milestone <RM>`.
3. **Split issues**: per sub-task, `add` (kind=requirement, label `<version>,dev-clean`) + `plan issue` to mount. Use `--priority` when creating. **After mounting, uniformly `state plan` to lock scheduling** (plan issues are always `planned`, never left `open`).
