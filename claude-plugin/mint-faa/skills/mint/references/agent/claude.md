...
| mem-lite → mint | observation text | `issue#<mint id>; read: mint show <id>` | `...（issue#3; read: mint show 3）` |
...
```bash
claude-mem-lite save "<content> (linked issue#<id>; read: mint show <id>)" \
...
2. Run `mint show <N>` to retrieve the full issue (default TSV with all fields incl. body).
3. For history / full scope: `mint list --all-states`.
