...
- **Body**: use `references/body-templates/N.md`, ≤4 fields, ≤1 sentence per field, bullet points with `-`. **No `- [ ]` checkboxes** (mint is lightweight — the agent does not re-edit the body, so a checkbox would forever show unchecked; use plain `- ` lists for steps/points). Common:
...
- **No version labels**: versions are expressed via plan→milestone; don't create version labels (existing ones have been cleaned up).
- **Module labels**: tag by development module (MCP/TUI/DB/CLI/plugin, etc.) to express the dimension kind fields don't cover; modules with no active issues auto-register on attach when needed.
