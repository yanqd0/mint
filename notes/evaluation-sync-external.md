...
- 比第一轮假设的 `INSERT OR IGNORE`（保留先到）更合理——先到 ≠ 更新
- **时钟漂移提示（#440）**：LWW 按 `updated_at` 判新，**跨机时钟偏差大时判新可能不准确**——时钟慢的机器新增可能被误判旧而被覆盖。低频同步场景影响小；必要时各机 NTP 对齐（UTC 存储 + 系统时钟同步缓解）
...
## 方案元评估：外调命令行（2026-08-18）

**结论**：外调命令行是当前约束下的正确架构决策。体积、扩展、安全、职责边界四维均优于内置；代价是系统复杂度外移 + 可观测性分离。成败取决于三条成功因素。

**体积**：基线 1.7M（strip，rusqlite bundled 占 .text 24.5%）；内置网络层 +0.5~3M（ureq ~+0.5-1M / reqwest+tokio +2-3M）；外调仅 +几十 KB（export=VACUUM INTO、merge=uid/LWW SQL、落地=symlink，全复用现有 rusqlite/std）。代价转移：用户侧需装 rclone/git/rsync——"mint 免依赖"成立、"同步系统"非单文件免依赖。

**扩展**：✅后端扩展最强（rclone 40+ 后端 = mint 零改动免费获得）；✅领域扩展在 mint 内（合并/落地策略可迭代）；⚠️传输增强受限（实时/双向/进度/状态报告——外部工具不懂 mint 语义，未来需回填传输日志）。

**可靠性/可观测性**：信号仅退出码 + 产物存在性；重试/进度/鉴权在外部，排障跨层 → mint 校验退出码 + 产物校验和 + 传输契约文档化。

**平台**：Windows 软链接权限受限 → 落地降级到路径切换；首次 `mv+ln` 迁移要幂等。

**安全**：凭据在外部（mint 安全面最小）✓；**spawn 必须 argv 数组禁 shell 拼接**（远端名/路径是注入面）。

**成功因素**：①export/merge 幂等+确定性+可校验；②传输契约文档化（命令/退出码/产物校验）；③降级路径明确（Windows→路径切换、无工具→文档引导）。
