...
- [领域概念词汇表](DDD.md) — Issue/Project/Label/Status（6 态 open/planned/dev/test/done/dropped）/Container（roadmap/plan 层级 + 5 态派生）/Issue Link（related/solves/duplicates）/Git 关联/capture/context/adapter 等核心概念与关系。
- [开发路线图](roadmap.md) — 版本规划（0.2.0 已完成 → 0.3 去重/claude → 0.4 TUI → 0.5 多机同步 → 0.6 体积优化 → 0.7 其它 agent → 1.0 发布含 i18n/docs）、发布策略（1.0 前公开预览）、每版目标。
- [技术选型与决策](decisions.md) — ADR 式记录（D1-D20）：命名/ORM/CLI 框架/SQLite 集成/体积目标/状态机/close 语义/语言策略/label/project 检测/容器建模/轻量迁移/issue links/容器 5 态派生/state commit。
